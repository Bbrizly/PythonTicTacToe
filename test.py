#Bassam Kamal | Mohammed Kamal | Sajed Fara|
...
milk,p1 = "",True
soy = int(input("\nWelcome to TicTacToe,\nYou can type stop to save and exit at any time.\nChoose: \n1. User Vs Computer \n2. User Vs User \n3. Resume Old Game\nChoose (1/2/3): "))
rows = [[e,e,e],[e,e,e],[e,e,e]]
while True:
    p1 = True
    if soy == 1:
        milk = input("Play as X or O? ").upper()
        if milk == o:
            Computer(x, 5)
            p1 = False
    elif soy == 3:
    ...
    while True:
        ...
        if rows[a][b] == e:
            if p1 == True:
                rows[a][b] = x
                if soy == 1 and milk == x: Computer(o,4)
                else: p1 = False
                if Wincheck(True): break #X then O
            elif p1 == False:
                rows[a][b] = o
                if soy == 1 and milk == o: Computer(x,5)
                else: p1 = True
                if Wincheck(False): break #O then X
            continue
        else:
